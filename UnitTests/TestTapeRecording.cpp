...
// StopRecord() closes the real API gap the original bug report flagged
// separately from the crash: Record() had no counterpart, so a wrapper
// arming it for a whole session could never cleanly end one (the
// "1.5 million tape inversions in 102 seconds" observation). Two real
// behaviours to prove, not just "it doesn't crash":
// 1. Calling it WHILE actively recording actually stops new inversions
//    from being appended -- IsRecordOn() goes false and nb_inversions_
//    stops growing even though ticks (with a still-toggling write line)
//    keep coming.
// 2. Calling it BEFORE Record()'s deferred start_record_ has ever
//    engaged cleanly cancels the arm -- recording never starts at all,
//    rather than starting anyway on the next tick.
